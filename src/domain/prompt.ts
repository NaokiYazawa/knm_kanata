...
1. \`mcp__kanata__report\` を \`kind: "progress"\` で呼び、やったこと (PR を作ったならその URL) を伝える
2. 続けて \`mcp__kanata__ask_human\` を \`question: "次は何をしますか？"\`, \`options: ["おわり"]\` で呼んで待つ
3. 続きの指示が返ってきたら、**同じセッションのまま**その作業を続ける (文脈は保たれています)
4. 「おわり」と返ってきたときだけ \`mcp__kanata__report\` を \`kind: "done"\` で呼んで終了する
