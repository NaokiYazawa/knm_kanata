...
#
#   PreToolUse (kanata のツールを呼ぶ直前) → コンテキスト使用量。表示する直前の値が要る
#   Stop       (ターンの終わり)            → コンテキスト使用量。質問を出さずに長く働くとき用
#   SessionEnd (セッションの終了)          → 完了通知の保険
#
# Stop を «終了» とみなしてはいけない。
# Stop は「Claude が応答を終えたとき」= 1 ターンごとに鳴る。終了は SessionEnd。ここを間違えると会話の途中で «終了しました» が出る (実際に出た)。
#
# コンテキスト量は **転写ログからしか取れない**。どの hook の入力にもトークン数は入っておらず、ステータスラインは対話 UI 専用でクラウドセッションでは動かない。
#
# 何があっても exit 0 する。
# hook が失敗すると Claude 側に赤が出て、本題と無関係なノイズになる。
